...
class TrainModule(pl.LightningModule):
    ...
    def __init__(
        ...
        metrics: List[str] = ['SDR', 'SI_SDR', 'NB_PESQ', 'WB_PESQ', 'eSTOI', 'DNSMOS'],
        ...
        self.reset = reset
        self.name = type(arch).__name__ + exp_name
        print(self.name)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # logging
        # self.log('val/' + self.loss.name, loss, sync_dist=True, batch_size=ys.shape[0])
        val_metric = {'loss': -loss, 'si_sdr': si_sdr_val, 'sdr': sdr_val, 'dnsmos': dnsmos_ovr.item()}[self.val_metric]
        ...
        # for idx, name in enumerate(['p808', 'sig', 'bak', 'ovr']):
        #     self.log(f'val/dnsmos_{name}', dnsmos_val[..., idx].mean(), sync_dist=True, batch_size=ys.shape[0])

...
class TrainModule(pl.LightningModule):
    ...
    import_path: str = 'SharedTrainerAlignWav.TrainModule'
    ...
    def __init__(
        ...
        stft_clean: STFT = STFT(n_fft=256, n_hop=64, win_len=256),
        norm: Norm = Norm(mode='utterance', online=False),
        loss: Loss = Loss(loss_func=neg_snr, pit=True),
    ...
    def forward(self, x: Tensor, clean: Tensor, istft: bool = True) -> Tuple[Tensor, Any]:
        """
        Args:
            x: [B,C,T]
            clean: [B,SPK,T]
        ...
        CLN, stft_paras_cln = self.stft_clean.stft(clean)  # [B,SPK,F,T], complex
        CLN, (CLNr, CLNrMM) = self.norm.norm(CLN, ref_channel=0)
        CLN = CLN.permute(0, 2, 3, 1)  # B,F,T,SPK; complex
        CLN = torch.view_as_real(CLN).reshape(*CLN.shape[:3], -1)  # B,F,T,2SPK
    ...
    def training_step(self, batch, batch_idx):
        ...
        x, ys, cln, paras = batch  # x: [B,C,T], ys: [B,Spk,C,T]
        yr = ys[:, :, self.ref_channel, :]

        yr_hat, loss_paras = self.forward(x, cln)
    ...
    def predict_step(self, batch: Union[Tensor, Tuple[Tensor, Tensor, Dict]], batch_idx: Optional[int] = None, dataloader_idx: Optional[int] = None) -> Tensor:
        ...
        # forward & loss
        # pad zeros for removing the clicking noise in the end part of some predicted audio
        x_pad = torch.concat([x, torch.zeros((x.shape[0], x.shape[1], 1600), device=x.device)], dim=-1)
        cln_pad = torch.concat([cln, torch.zeros((cln.shape[0], cln.shape[1], 1600), device=x.device)], dim=-1)
        yr_hat_pad, loss_paras = self.forward(x_pad, cln_pad)
        yr_hat = yr_hat_pad[..., :-1600]

        if self.loss.is_scale_invariant_loss:
            x_ref = x[:, self.ref_channel, :]
            yr_hat = recover_scale(preds=yr_hat, mixture=x_ref, scale_src_together=True if self.loss.loss_func == neg_sa_sdr else False, norm_if_exceed_1=False)

        if yr is not None and yr.shape[1] > 1:  # reorder yr_hat if given yr
    ...
    def on_predict_batch_end(self, outputs: Optional[Any], batch: Any, batch_idx: int, dataloader_idx: int = 0) -> None:
        GS.on_predict_batch_end(self=self, outputs=outputs, batch=(batch[0], batch[1], batch[3]))
...
if __name__ == '__main__':
    # python -m models.SharedTrainerAlignWav predict --config=logs/AlignModel/version_6/config.yaml --config=configs/datasets/realistic_audio.yaml --ckpt_path=logs/AlignModel/version_6/checkpoints/epoch104_metric33.0974.ckpt --trainer.devices=7, --trainer.precision=32 --data.batch_size=\[1,2\] --model.compile=false --data.audio_time_len=\[null,null,null,null\]
    cli = TrainCLI(
        TrainModule,

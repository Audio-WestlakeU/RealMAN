...
class BF_module(DPRNN_base):
    def __init__(self, *args, **kwargs):
        ...
        self.output_gate = nn.Sequential(nn.Conv1d(self.feature_dim, self.output_dim, 1), nn.Sigmoid())
...
class FaSNet_base(nn.Module):
    def __init__(self, enc_dim, feature_dim, hidden_dim, layer, segment_size=50, nspk=2, win_len=4, context_len=16, sr=16000):
    ...
    def pad_input(self, input, window):
        ...
        if rest > 0:
            pad = torch.zeros(batch_size, nmic, rest).type(input.dtype).to(input.device)
            input = torch.cat([input, pad], 2)
        pad_aux = torch.zeros(batch_size, nmic, stride).type(input.dtype).to(input.device)
    ...
    def seg_signal_context(self, x, window, context):
        ...
        pad_context = torch.zeros(batch_size, nmic, context).type(input.dtype).to(input.device)
        ...
        begin_idx = torch.from_numpy(begin_idx).type(input.dtype).to(input.device).long().view(1, 1, -1)  # 1, 1, nchunk
    ...
    def seq_cos_sim(self, ref, target):
        ...
        # L2 norms
        ref_norm = F.conv1d(ref.view(1, -1, ref.size(2)).pow(2), torch.ones(ref.size(0) * ref.size(1), 1, target.size(2)).type(ref.dtype).to(ref.device), groups=larger_ch * seq_length)  # 1, larger_ch*L, seg1-seg2+1
        ref_norm = ref_norm.sqrt() + self.eps
        target_norm = target.norm(2, dim=2).view(1, -1, 1) + self.eps  # 1, larger_ch*L, 1
        # cosine similarity
        cos_sim = F.conv1d(ref.view(1, -1, ref.size(2)), target.view(-1, 1, target.size(2)), groups=larger_ch * seq_length)  # 1, larger_ch*L, seg1-seg2+1
...
class FaSNet_origin(FaSNet_base):
    def __init__(self, *args, **kwargs):
        ...
        self.other_BF = BF_module(self.filter_dim + self.enc_dim, self.feature_dim, self.hidden_dim, self.filter_dim, 1, self.layer, self.segment_size, model_type='DPRNN')
    ...
    def forward(self, input, num_mic):
        ...
        ref_output = F.conv1d(ref_context.view(1, -1, self.context * 2 + self.window), ref_filter.view(-1, 1, self.filter_dim), groups=batch_size * self.num_spk * seq_length)  # 1, B*nspk*L, win
        ...
        other_output = F.conv1d(other_context_saved.view(1, -1, self.context * 2 + self.window), other_filter.view(-1, 1, self.filter_dim), groups=batch_size * self.num_spk * (nmic - 1) * seq_length)  # 1, B*nspk*(nmic-1)*L, win
...
class FaSNet_TAC(FaSNet_base):
    def __init__(self, *args, **kwargs):
        ...
        self.all_BF = BF_module(self.filter_dim + self.enc_dim, self.feature_dim, self.hidden_dim, self.filter_dim, self.num_spk, self.layer, self.segment_size, model_type='DPRNN_TAC')
    ...
    def forward(self, input, num_mic):
        ...
        all_bf_output = F.conv1d(mic_context.view(1, -1, self.context * 2 + self.window), all_filter.view(-1, 1, self.filter_dim), groups=batch_size * nmic * self.num_spk * seq_length)  # 1, B*nmic*nspk*L, win
...
def test_model(model):
    ...
    num_mic = torch.from_numpy(np.array([3, 2])).view(-1,).type(x.type())  # ad-hoc array，每个batch使用几个通道的意思（前n个）
...
if __name__ == "__main__":
    # python -m models.fasnet.FaSNet
    sr = 8000
    model_origin = FaSNet_origin(enc_dim=64, feature_dim=64, hidden_dim=128, layer=6, segment_size=50, nspk=2, win_len=4, context_len=16, sr=sr)

    model_TAC = FaSNet_TAC(enc_dim=64, feature_dim=64, hidden_dim=128, layer=4, segment_size=50, nspk=2, win_len=4, context_len=16, sr=sr)

    # test_model(model_origin)
    test_model(model_TAC)
    print(model_TAC)

    model = model_TAC.to('meta')
    x = torch.rand(1, 6, sr * 4).to('meta')
    none_mic = np.zeros((1,))  # torch.zeros(1).type(x.dtype).to('meta')
    from torch.utils.flop_counter import FlopCounterMode
    from models.utils.flops import detailed_flops
    with FlopCounterMode(model, display=False) as fcm:
        y = model(x, none_mic)
        flops_forward_eval = fcm.get_total_flops()
        print(detailed_flops(fcm))
        res = y.sum()
        res.backward()
    ...
    # 8k 6chn: flops_forward=106.89G, flops_back=9703.93G, params=2.69 M
    # 16k 6chn: flops_forward=111.67G, flops_back=38113.45G, params=2.76 M
    # 8k 2chn: flops_forward=38.85G, flops_back=1131.54G, params=2.69 M
    # 16k 2chn: flops_forward=40.45G, flops_back=4289.12G, params=2.76 M

...
class DPRNN_base(nn.Module):
    ...
    def pad_segment(self, input, segment_size):
        ...
        if rest > 0:
            pad = Variable(torch.zeros(batch_size, dim, rest)).type(input.dtype).to(input.device)
            input = torch.cat([input, pad], 2)
        
        pad_aux = Variable(torch.zeros(batch_size, dim, segment_stride)).type(input.dtype).to(input.device)
